...
const state = { projects: [], projectId: '', project: null, sources: [], assets: [], storyboard: [] };
...
async function loadProjects(){
  ...
  $('project-list').innerHTML = state.projects.map(p => `
    <button class="project ${p.id===state.projectId?'active':''}" data-project-id="${p.id}">
      <b>${escapeHtml(p.name)}</b><span>${escapeHtml(p.project_type || '宣传片')} · ${escapeHtml(p.status || '')}</span>
    </button>`).join('') || '<p class="empty">暂无项目</p>';
...
async function selectProject(projectId){
  ...
  const [project, brief, sources, assets, readiness] = await Promise.all([
    WorkflowAPI.project(projectId), WorkflowAPI.brief(projectId), WorkflowAPI.sources(projectId), WorkflowAPI.assets(projectId), WorkflowAPI.readiness(projectId)
  ]);
  ...
  debug({project, brief, sources, assets, readiness});
}
...
async function saveBrief(){
  ...
  const payload = {
    ...
    target_audience: $('brief-audience').value,
    tone: $('brief-tone').value,
    required_messages: $('brief-required').value,
    forbidden_expressions: $('brief-forbidden').value,
  };
  const data = await WorkflowAPI.saveBrief(state.projectId, payload);
  status('brief-status', '已保存', 'success');
  debug(data);
...
function renderReadiness(data){
  const c = data.counts || {};
  const metrics = [
    ['资料', c.sources || 0, c.sources && c.parsed_sources === c.sources ? 'ok' : 'warn'],
    ['已解析资料', c.parsed_sources || 0, c.parsed_sources ? 'ok' : 'warn'],
    ['视觉资产', c.assets || 0, c.assets && c.authorized_assets === c.assets ? 'ok' : 'warn'],
    ['结构化镜头', c.structured_scenes || 0, c.structured_scenes ? 'ok' : 'bad'],
  ];
...
function renderNextActions(data){
  const blockers = data.blockers || [];
  const actions = blockers.length ? blockers : ['完善视觉资产授权', '生成结构化分镜', '进入图片/视频生成队列', '发布前运行自检并导出证据包'];
...
function renderSources(){
  $('source-list').innerHTML = state.sources.map(s => `
    <div class="source-card" data-source-card="${s.id}">
      <div class="source-head"><div class="trusted-mark">S</div><div><b>${escapeHtml(s.title || s.original_name || '未命名资料')}</b><small>${escapeHtml(s.file_type || s.source_type || '')} · ${escapeHtml(s.parse_status || 'pending')}</small></div></div>
      <div><span class="pill ${s.can_quote?'ok':'bad'}">${s.can_quote?'可引用':'不可引用'}</span><span class="pill ${s.can_visualize?'ok':'warn'}">${s.can_visualize?'可视化':'不建议可视化'}</span><span class="pill">${escapeHtml(s.source_authority_level || 'internal')}</span><span class="pill ${s.sensitive_level==='high'?'bad':s.sensitive_level==='medium'?'warn':'ok'}">${escapeHtml(s.sensitive_level || 'normal')}</span></div>
      <div class="mini-grid" style="margin-top:10px">
        <div class="field"><label>权威等级</label><select data-field="source_authority_level"><option value="official">官方资料</option><option value="internal">内部资料</option><option value="media">媒体资料</option><option value="user_input">用户输入</option></select></div>
        <div class="field"><label>敏感等级</label><select data-field="sensitive_level"><option value="normal">普通</option><option value="medium">中等</option><option value="high">高敏感</option></select></div>
        <div class="field"><label>来源责任人</label><input data-field="source_owner" value="${escapeHtml(s.source_owner || '')}"></div>
      </div>
      <div class="mini-grid">
        <div class="field"><label>是否可引用</label><select data-field="can_quote"><option value="true">可引用</option><option value="false">不可引用</option></select></div>
        <div class="field"><label>是否可视化</label><select data-field="can_visualize"><option value="true">可视化</option><option value="false">不可视化</option></select></div>
        <div class="field"><label>引用是否必需</label><select data-field="citation_required"><option value="true">必需</option><option value="false">非必需</option></select></div>
      </div>
      <div class="field"><label>可信度备注</label><textarea data-field="notes">${escapeHtml(s.notes || '')}</textarea></div>
      <div class="table-actions"><button class="btn" data-parse-source="${s.id}">解析该资料</button><button class="btn primary" data-save-source="${s.id}">保存可信度</button></div>
    </div>`).join('') || '<div class="card"><div class="card-body"><p class="empty">暂无资料。先上传政策文件、甲方确认稿、图片或视频参考。</p></div></div>';
  state.sources.forEach(s => {
    const card = document.querySelector(`[data-source-card="${s.id}"]`);
    if(!card) return;
    card.querySelector('[data-field="source_authority_level"]').value = s.source_authority_level || 'internal';
    card.querySelector('[data-field="sensitive_level"]').value = s.sensitive_level || 'normal';
    card.querySelector('[data-field="can_quote"]').value = String(!!s.can_quote);
    card.querySelector('[data-field="can_visualize"]').value = String(!!s.can_visualize);
    card.querySelector('[data-field="citation_required"]').value = String(s.citation_required !== false);
  });
...
async function saveSourceTrust(sourceId){
  ...
  const data = await WorkflowAPI.saveSourceTrust(sourceId, payload);
  status('source-status', '资料可信度已保存', 'success');
  debug(data);
  await refreshSources();
}

async function parseOneSource(sourceId){
  const data = await fetch(`/api/workflow/sources/${sourceId}/parse`, {method:'POST', headers:{Authorization:`Bearer ${tokenStore.get()}`}}).then(async r => {
    const d = await r.json(); if(!r.ok) throw new Error(d.error || d.message || r.statusText); return d;
  });
  status('source-status', '资料已提交解析', 'success');
  debug(data);
  await refreshSources();
}
...
async function uploadSource(){
  ...
  const data = await WorkflowAPI.uploadSource(state.projectId, file, $('upload-data-level').value);
  status('source-status', '资料已上传', 'success');
  debug(data);
  await refreshSources();
}
...
async function parseAllSources(){
  requireProject();
  const data = await WorkflowAPI.parseAllSources(state.projectId);
  status('source-status', '资料批量解析完成', 'success');
  debug(data);
  await refreshCurrent();
}
...
async function createAsset(){
  requireProject();
  const payload = {title:$('asset-title').value || '未命名资产', asset_type:$('asset-type').value, auth_status:$('asset-auth').value, visual_description:$('asset-desc').value};
  const data = await WorkflowAPI.createAsset(state.projectId, payload);
  $('asset-title').value = ''; $('asset-desc').value = '';
  status('asset-status', '资产已新增', 'success');
  debug(data);
  await refreshAssets();
}

function sceneToStoryboard(s){
  return {scene_id:s.id, scene_name:s.name, scene_goal:s.scene_goal, source_citations:parseJson(s.source_citations_json, []), shot_size:s.shot_size, camera_movement:s.camera_movement, generation_mode:s.generation_mode, visual_subject:s.visual_subject, location:s.location, subtitle_text:s.subtitle_text, image_prompt:'', video_prompt:s.prompt || s.voiceover_text || ''};
}
function parseJson(value, fallback){ try{return JSON.parse(value || '')}catch{return fallback} }

function renderStoryboard(items){
  $('storyboard-list').innerHTML = items.map((s,i) => `
    <div class="shot-card"><div class="shot-head"><div class="trusted-mark">${String(i+1).padStart(2,'0')}</div><div><b>${escapeHtml(s.scene_name || '镜头')}</b><small>${escapeHtml(s.scene_goal || '')}</small></div></div>
      <div class="shot-grid"><div class="kv"><span>资料引用</span><b>${escapeHtml((s.source_citations||[]).join('、') || '待补')}</b></div><div class="kv"><span>景别 / 运镜</span><b>${escapeHtml((s.shot_size||'')+' / '+(s.camera_movement||''))}</b></div><div class="kv"><span>生成模式</span><b>${escapeHtml(s.generation_mode || '')}</b></div><div class="kv"><span>视觉主体</span><b>${escapeHtml(s.visual_subject || '')}</b></div><div class="kv"><span>场景</span><b>${escapeHtml(s.location || '')}</b></div><div class="kv"><span>字幕</span><b>${escapeHtml(s.subtitle_text || '')}</b></div></div>
      <div class="prompt">图片 Prompt：\n${escapeHtml(s.image_prompt || '已保存的旧镜头暂无图片 Prompt，请重新生成结构化分镜。')}\n\n视频 Prompt：\n${escapeHtml(s.video_prompt || '')}</div>
    </div>`).join('') || '<p class="empty">暂无结构化分镜。先生成脚本或直接点击“生成结构化分镜”。</p>';
}

async function generateStoryboard(){
  requireProject();
  const data = await WorkflowAPI.structuredStoryboard(state.projectId, {style:$('story-style').value || '纪实、克制、可信、公共服务宣传片'});
