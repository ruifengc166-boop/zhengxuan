...
def create_project():
    ...
    if not org_id:
        return jsonify({"error": "当前账号未绑定组织，无法创建单位项目"}), 400
    ...
    db.execute(
        ...
        INSERT INTO projects (
            id,name,org_id,user_id,project_type,deadline,data_level,rule_pack,publish_channel,aspect_ratio,duration_target
        ) VALUES (?,?,?,?,?,?,?,?,?,?,?)
        """,
        (
            pid,
...
def upload_file():
    ...
    db.execute(
        ...
        INSERT INTO uploaded_files (
            id,filename,original_name,file_type,file_size,uploader_id,project_id,org_id,mime_type,data_level,storage_status
        ) VALUES (?,?,?,?,?,?,?,?,?,?,?)
        """,
        (
            fid,
    ...
    if project_id:
        db.execute(
            ...
            INSERT INTO project_sources (
                id,project_id,org_id,file_id,title,source_type,data_level,parse_status,created_by
            ) VALUES (?,?,?,?,?,?,?,?,?)
            """,
    ...
    log_current_user("upload_file", f"上传文件 {safe_original} 到项目 {project_id or '-'}")
...
def create_generation_task(task_type, payload):
    ...
    db.execute(
        ...
        INSERT INTO generation_tasks (
            id,org_id,project_id,scene_id,task_type,status,progress,provider,model_name,api_source,
            prompt,params_json,estimated_cost,created_by,updated_at
        ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """,
        (
            task_id,
...
def generate_images():
    data = request.get_json() or {}
    result, error = create_generation_task("image", data)
    if error:
        return error
...
def generate_videos():
    data = request.get_json() or {}
    result, error = create_generation_task("video", data)
    if error:
        return error
    return jsonify(result), 202
